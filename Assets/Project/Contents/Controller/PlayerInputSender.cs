...
namespace Icarus.Controller
{
    ...
    public class PlayerInputSender : BaseNetworkActor, ITickable//, IControllerSetter
    {
        private InterfaceReceiver<IMoveInput> _moveInputReceiver = new();
        private InterfaceReceiver<ICameraRotationProvider> _CameraRotationProviderReceiver = new();

        IMoveInput _moveInput;
        IMoveInput MoveInput => InterfaceHelper.GetInterface(ref _moveInput, _moveInputReceiver);

        ICameraRotationProvider _cameraProvider;
        ICameraRotationProvider CameraProvider => InterfaceHelper.GetInterface(ref _cameraProvider, _CameraRotationProviderReceiver);
        ...
        public override void OnStopClient()
        {
            ...
            _CameraRotationProviderReceiver.Unbind();
        }
        public override void OnStartClient()
        {
            ...
            _CameraRotationProviderReceiver.Bind();
        }

        public void Tick(float deltaTime)
        {
            if (MoveInput is not null) InputMove();
        }

        private void InputMove()
        {
            Vector2 rawMove = MoveInput.value;
        ...
        private Vector2 GetCameraRelativeDirection(Vector2 worldFixedMove)
        {
            ...
            if (CameraProvider is not null)
            {
                ...
                float camY = CameraProvider.WorldYRotation;
